const Config = {
  inventory: "LOST-inventory", // Change this to your inventory resource name
};
...
$(function () {
  ...
  document.onkeyup = function (data) {
    if (data.which == 27) {
      $.post("https://LOST-crafting/exit", JSON.stringify({}));
      $counter.val(1);
      $incButton.off("click");
      $decButton.off("click");
      return;
...
async function loadCraftingTable(tableName) {
  const response = await fetch("https://LOST-crafting/items", {
    method: "POST",
    headers: {
